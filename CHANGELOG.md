...
- **修复（#1103/#1114，RSS 匹配）**：解析器重组的 `title_raw`（剔除标题中间的集数、拼接 ` / ` 分隔的别名）不是种子名的子串，导致新剧集无法匹配、被归入孤儿种子；现在与种子自身重组出的标题完全相等也算命中，已有数据无需迁移
- **修复（#1105，订阅）**：搜索类 RSS 同时返回多季资源时，订阅/收集整订阅下载会把其他季的种子按本季下载并重命名，互相覆盖；现在按季度/类型过滤，与 RSS 刷新的匹配规则一致
- **修复（#1104，网络）**：HTTP 代理下 CONNECT 成功但 TLS 握手失败时，httpcore 泄漏隧道连接，连接池耗尽后持续 `PoolTimeout` 直至重启；现在遇到 `PoolTimeout` 时为后续请求换用新连接池，不中断进行中的请求
- **修复（#1108，解析器）**：Unicode 罗马数字（`Ⅱ` 等）识别为季度，去除包裹中文标题的全角破折号，`Clevatess Ⅱ－魔獸之王與虛假的勇者傳承－` 解析为第 2 季并可匹配 TMDB
- **修复（#1106，Docker）**：健康检查改为读取配置的 WebUI 端口，依次探测 IPv4 与 IPv6 回环地址，不再因 `localhost` 解析为 `::1` 或修改端口而被标记为 unhealthy
- **修复（#1117，日志）**：RSS/重命名间隔配置为 0 时后台循环空转刷屏，现在最小间隔为 1 秒；部署文档中的 docker-compose 与 `docker run` 示例增加容器日志大小上限（10 MB × 3）

# [3.3.5] - 2026-07-26

3.3.5 是问题修复版本，包含五个订阅、通知与配置相关的修复。

- **修复（#1095，RSS）**：删除番剧后重新订阅同一 RSS 时，自动重新启用被孤儿停用机制禁用的订阅，番剧不再静默断更
- **修复（#1094，通知）**：Telegram 海报默认改为发送源 URL（由 Telegram 服务端拉图），失败时降级为本地上传、再降级为纯文本消息；下载完成通知不再因海报发送失败而整条丢失，服务端拒绝（4xx/5xx）与连接失败的日志分开记录并附带响应内容
- **修复（#1092，解析器）**：classic 引擎与 Preview 准入策略对齐，接纳字幕组未标记特别篇/剧场版且无集数、但带发布证据（组名/分辨率/片源）的单发资源，按整理集处理；纯文字弱标题仍被拒绝
- **修复（数据库）**：「重置所有规则」先清理种子记录与 aria2 映射的外键引用再删除番剧，不再因 `FOREIGN KEY constraint failed` 整体回滚；重置失败残留的去重记录导致番剧补全跳过已有集数的问题随之解决
- **修复（配置）**：未设置的密码/密钥不再在界面上显示为 `********` 幻影掩码；自动清洗 3.2.3/3.2.4 版本窗口期间被字面写入配置的掩码哨兵（该窗口只掩码不恢复，导致代理/下载器认证失败），清洗时告警提示重新输入

# [3.3.4] - 2026-07-13

3.3.4 正式版汇总 beta.1–beta.2。经典解析器仍为默认；通用解析器 Preview 与修订版自动替换均需手动启用。

- **新增（#1074，认证）**：引入持久化多用户会话、首次管理员初始化、用户管理，以及按 `api` / `mcp` scope 隔离且仅展示一次明文的哈希令牌；密码变更、禁用用户和退出登录会撤销相关会话，浏览器刷新仅续期 HttpOnly Cookie
- **新增（Parser Preview）**：解析设置可在稳定的经典解析器与通用解析器 Preview 间切换；Preview 支持剧集、OVA/特别篇、剧场版、范围和混合合集，两种模式不会自动互相回退
- **修复（#1078）**：更高修订版与既有文件冲突时，默认 `hold` 记录并通知冲突；可选 `replace` 在严格匹配番剧、媒体类型、季集、字幕组和分辨率后，通过 qBittorrent/aria2 的可恢复三阶段流程替换旧版。自动替换首版仅处理双方均为单文件的任务
- **修复（#1077）**：日历不再显示已归档番剧
- **修复（#1072/#1075，解析）**：`06v2` 等修正版集数不再误触发 offset review；TMDB 电视与电影查询遵循配置语言；剧场版标题中的有效 `Movie` 和中日韩电影词不再被误删
- **CI/发布**：新增隔离的运行时、Chromium/WebKit 与真实 qBittorrent E2E lanes；release 分类仅依据 event/ref/tag，正式版 tag 必须位于 `main`

# [3.3.4-beta.2] - 2026-07-13

- **修复（#1078）**：同一番剧、媒体类型、季集、字幕组和分辨率出现更高修订版时，不再因目标文件已存在而反复重命名；默认 `hold` 保留双方并记录冲突，可选 `replace` 安全替换旧版本
- **新增（修订版替换）**：qBittorrent 与 aria2 使用可恢复的“暂存旧版 → 晋升新版 → 清理旧版”流程；异常重启、目标冲突和下载任务消失时可继续或回滚，首版只自动处理双方均为单文件的任务
- **新增（管理与通知）**：设置页增加全局修订冲突策略，新增冲突查询/重试 API 和通知中心事件
- **安全性**：多文件合集、身份信息不完整、非严格升级或同一路径存在多个下载器 owner 时一律保持 `hold`，不会自动删除旧任务或文件
- **测试**：覆盖 V1/V2 严格身份匹配、并发抢占、断点恢复、合集保护、qBittorrent/aria2 冲突与数据库迁移

# [3.3.4-beta.1] - 2026-07-10

- **新增（#1074，认证）**：引入多用户会话认证——首次安装创建管理员，支持用户管理、独立浏览器会话，以及按 `api` / `mcp` scope 隔离且仅展示一次明文的哈希令牌；密码变更、禁用用户和退出登录都会可靠撤销既有会话
- **修复（认证安全）**：刷新接口只续期 HttpOnly Cookie，不再向响应体或浏览器存储暴露令牌；一次性 API Token 在弹窗关闭、组件离开和延迟响应竞态下都会立即清除，移动端补齐 44px 可访问关闭按钮
- **修复（#1075）**：TMDB 电视与电影搜索严格使用解析设置中的语言；切换语言或修剪输入空白后，会以对应 locale 重新查询且不复用错误缓存
- **修复（#1072）**：`06v2` 等修正版集数与 CRC 后缀不再被误判为季/集偏移异常；真实超范围集数仍会进入 offset review
- **修复（解析器）**：剧场版标题中的 `Movie` 等有效词不再被重复或误删，只移除末尾真正的发布类型标记
- **修复（#1065，发布）**：release trigger 分类改为完全由 event/ref/tag 决定，PR 标题不再影响版本；稳定 tag 仍必须位于 `main`，beta tag 可在 dev/测试分支安全发布
- **测试/CI**：新增完全隔离的运行时、Chromium、WebKit 与真实 qBittorrent E2E lanes，覆盖认证、重启持久化、Windows 路径、TMDB 语言、offset review、移动端规则编辑、播放器本地设置，以及 RSS 下载→校验→重命名→删除全链路

# [3.3.3] - 2026-07-09

- **新增**：AniBT 作为内置搜索源，默认使用 `https://anibt.net/rss/magnets.xml?q=%s` 并走 TMDB 解析；旧配置文件加载时会自动补齐新增的内置搜索源，同时保留用户已有搜索源的自定义 URL / 解析器
- **修复**（#1067）：搜索结果预览会按“解析设置”中的 TMDB 语言重新取标题，不再在设置为日语等语言时回落到中文/英文标题；海报补全复用同一次 TMDB 预览请求并按语言缓存
- **修复**（#1068）：删除/禁用番剧并选择 `file=true` 时，匹配 qBittorrent 种子的 `save_path` 会统一归一化路径分隔符，Windows qB 返回反斜杠路径时也能正确删除种子文件
- **修复**（#1069，WebUI）：手机端编辑规则弹窗改为全屏底部面板，并关闭会把面板整体上推的软键盘补偿，避免作品名称输入框被悬浮栏/软键盘挤出可编辑区域
- **修复**（CI）：稳定版发布只接受打在 `main` 上的语义化版本 tag，dev 分支仅允许 alpha/beta 预发布 tag，避免合并 PR 时误触发正式发布

# [3.3.2] - 2026-07-07

- **修复**：qBittorrent ≤5.1 对 .torrent 文件上传回笼统的 `200 "Fails."` 时，用文件字节计算 infohash 到 qB 确认是否已存在——已存在归类为重复并标记已下载，不再每轮 RSS 刷新无限重试报错（"Fails." … "no matching torrent found" 循环，常见于清空孤儿记录后种子仍在 qB 的场景）
- **修复**（WebUI）：重复搜索时旧的搜索流未关闭——结果被重复追加（重复的条目/tag）、旧流无限自动重连导致输入框为空时仍持续转圈；现在开新搜索前先关旧流、追加结果按订阅链接去重、清空输入即停止搜索
