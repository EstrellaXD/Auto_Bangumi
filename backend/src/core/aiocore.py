...
class AsyncApplicationCore:
    ...
    def __init__(self):
        ...
        self._download_check_monitor = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # 应该等一下 download 登陆, 像是rename呀, rss都要用到下载服务
    ...
    async def _register_event_handlers(self) -> None:
        ...
        try:
            ...
            # 使用全局 DownloadCheckMonitor 实例（需要先于 DownloadMonitor 初始化）
            self._download_check_monitor = download_check_monitor
            self._download_check_monitor.initialize()
    ...
    async def stop(self) -> None:
        ...
        # 关闭监控器
        try:
            if hasattr(self, "_download_check_monitor") and self._download_check_monitor:
                await self._download_check_monitor.shutdown()
                logger.debug("[AsyncCore] 下载检查监控器已关闭")
        except Exception as e:
            logger.error(f"[AsyncCore] 下载检查监控器关闭失败: {e}")
