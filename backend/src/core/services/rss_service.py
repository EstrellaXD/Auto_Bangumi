...
class RSSService(BaseService):
    ...
    async def _setup(self) -> None:
        ...
        self._eps_complete_enabled = get_rename_config().eps_complete
    ...
    def get_task_config(self) -> dict[str, Any]:
        ...
        return {
            "name": "rss_refresh",
            "interval": get_program_config().rss_time,
            "enabled": True,
        }
    ...
    async def execute(self) -> None:
        ...
        try:
            ...
            if self._eps_complete_enabled:
                ...
                try:
                    await asyncio.wait_for(download_queue.join(), timeout=20)
                    await eps_complete()
                except asyncio.TimeoutError:
                    logger.warning("[RSSService] 当前任务过多，跳过Eps Complete处理")
