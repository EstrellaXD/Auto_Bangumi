...
def verify_bundle_signature(
    ...
    try:
        ...
        logger.error("cryptography unavailable; refusing unverified bundle")
        return False

    try:
        public_key = load_pem_public_key(pubkey_path.read_bytes())
        if not isinstance(public_key, Ed25519PublicKey):
            logger.error("pubkey is not ed25519; refusing bundle")
            return False
        ...
        logger.error("cannot verify bundle signature: %s", exc)
        return False
