...
_rename_config: BangumiManage | None = None


def get_rename_config() -> BangumiManage:
    """获取重命名配置，如果未初始化则返回默认配置"""
    if _rename_config is None:
        return BangumiManage()
    return _rename_config


def set_rename_config(config: BangumiManage):
    """设置重命名配置"""
    global _rename_config
    _rename_config = config
...
class Renamer:
    ...
    async def rename_file(
        ...
        rename_method = get_rename_config().rename_method
