...
class NotificationManager:
    ...
    async def send_notification(
        self, notification: Notification, use_retry: bool = True
    ) -> Dict[str, bool]:
        """
        发送通知到所有启用的通知器
        ...
        if success_count > 0:
            self.logger.info(
                f"通知发送完成: {notification.title} ({success_count}/{total_count} 成功)"
            )
        else:
    ...
    async def _send_single_notification(
        ...
        try:
            ...
            if use_retry:
                ...
                result = await asyncio.wait_for(
                    notifier.post_msg(notification), timeout=config.timeout
                )
