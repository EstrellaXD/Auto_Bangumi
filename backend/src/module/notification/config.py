...
class NotificationConfigManager:
        ...
        for config_data in configs_data:
            try:
                ...
                config = NotificationConfig(
                    ...
                    retry_count=config_data.get('retry_count', settings.default_retry_count),
    ...
    def remove_config(self, config_type: NotificationType, token: str = None) -> int:
        ...
        self.settings.configs = [
            c for c in self.settings.configs
    ...
    def get_configs_by_type(self, config_type: NotificationType) -> List[NotificationConfig]:
        """根据类型获取配置"""
...
if __name__ == "__main__":
    # 测试配置管理器
    config_file = Path("test_notification_config.json")
    
    # 创建测试配置
    manager = NotificationConfigManager(config_file)
    manager.settings = create_default_config()
    ...
    manager.add_config(telegram_config)
    
    # 保存和加载测试
    if manager.save_to_file():
        print("配置保存成功")
        print(f"配置摘要: {manager.get_summary()}")
        
        # 重新加载
        new_manager = NotificationConfigManager(config_file)
        print(f"重新加载后的摘要: {new_manager.get_summary()}")
