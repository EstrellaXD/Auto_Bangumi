...
} = useTorrentList(() => apiBangumi.getOrphanTorrents());

async function deleteOne(id: number) {
  await runDelete(() => apiBangumi.deleteOrphanTorrent(id), `已删除种子 ${id}`);
}

async function deleteSelected() {
  const ids = [...selectedIds.value];
  await runDelete(
    () => Promise.all(ids.map(id => apiBangumi.deleteOrphanTorrent(id))),
    `已删除 ${ids.length} 条种子`,
  );
}
...
async function confirmClear() {
  ...
  await runDelete(() => apiBangumi.deleteOrphanTorrents(), '已清空所有未匹配种子');
}
...
<template>
  <div class="page-torrents">
    <div class="toolbar">
      <h2 class="toolbar-title">{{ $t('homepage.others.title') }}</h2>
