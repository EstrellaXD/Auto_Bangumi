...
async function handleDeleteOne(id: number) {
  await runDelete(() => props.deleteOne(id), t('homepage.torrents.deleted_one', { id }));
}
  ...
  <div class="page-torrents">
    <div class="toolbar">
      ...
      <div class="toolbar-actions">
        <button v-if="torrents.length > 0" class="toolbar-btn" @click="toggleAll">
          ...
          {{ allSelected ? $t('homepage.torrents.deselect_all') : $t('homepage.torrents.select_all') }}
        </button>
        <button v-if="selectedIds.size > 0" class="toolbar-btn toolbar-btn--danger" @click="handleDeleteSelected">
          <Delete :size="16" />
    ...
    <div v-else class="torrent-scroll">
      ...
      >
        ...
        <button class="btn-delete" :aria-label="$t('homepage.torrents.delete')" @click.stop="handleDeleteOne(torrent.id)">
          <Close :size="14" />
        </button>
      </div>
    </div>
    ...
    <ab-popup v-model:show="showConfirmClear" :title="$t('homepage.torrents.deleteAll')">
      <div class="confirm-body">
        ...
        <div class="confirm-actions">
          ...
          <ab-button size="small" type="warn" @click="handleClearAll">{{ $t('homepage.torrents.deleteAll') }}</ab-button>
        </div>
      </div>
    </ab-popup>
  </div>
